...
import { Body, Caption, Extra, Title } from '@/constants/Typography/TypographyList';
        ...
        <div key={item.name}>
          ...
          <div className='max-w-[400px] mx-auto gap-2 flex flex-row justify-center'>
            <div className='text-center py-2 px-4'>
              ...
              <div className={`${Body.body2}`}>D-{calculateRemainingDays(item.adoptionDate)}</div>
            </div>
            <div className='text-center  py-2 px-4'>
              <Image src={born} alt='profile' width={44} height={44} />
              <span className={`${Caption.caption2}  text-gray-500`}>태어난지</span>
              <div className={`${Body.body2}`}>{calculateElapsedDays(item.adoptionDate)}일</div>
            </div>
            <div className='text-center  py-2 px-4'>
