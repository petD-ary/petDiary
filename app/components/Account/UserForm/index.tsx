...
const UserForm = () => {
  ...
  const handleCheckedEmail = async () => {
    if (email !== "") {
      ...
      querySnapshot.forEach((doc) => userIdList.push({ id: doc.id, ...doc.data() }));

      /* 서버에서 받아온 사용자 정보로 email 유효성 검사 */
      const result = userIdList.filter((item) => item.email === email);
      setEmailCheck((prev) => (result.length === 0 ? true : prev === null ? false : prev));
    }
  };

  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const userInfo = { userId, email, password };
    setStep((prev) => prev + 1);
  ...
  return (
    <form onSubmit={(e) => handleSubmit(e)} className="w-full pt-6 pb-16">
      <InputButton
        ...
        type="text"
        value={userId}
        setValue={(value) => setAuthObj((authObj) => ({ ...authObj, userId: value }))}
      ...
      <ShowInput
        ...
        value={password}
        setValue={(value) => setAuthObj((authObj) => ({ ...authObj, password: value }))}
      ...
      <InputCheck
        ...
        type="password"
        value={passwordCheck}
        setValue={(value) => setAuthObj((authObj) => ({ ...authObj, passwordCheck: value }))}
